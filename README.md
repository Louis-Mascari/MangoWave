...
- **CI/CD:** GitHub Actions — CI runs on push to `main` and PRs (lint + unit tests → E2E). Deploy triggers automatically after CI succeeds on `main` via `workflow_run` (CDK → S3/CloudFront). E2E runs Chromium + mobile Chrome full suite, Firefox + WebKit start-screen only; all 5 browsers locally
