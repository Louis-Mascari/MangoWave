...
export default class Renderer {
  ...
  runPixelEquations(preset, mdVSFrame, runVertEQs, blending) {
    ...
    for (let iz = 0; iz < gridZ1; iz++) {
      for (let ix = 0; ix < gridX1; ix++) {
        ...
        if (warp !== 0) {
          u +=
            warp * 0.0035 * Math.sin(warpTimeV * 0.333 + warpScaleInv * (x * warpf0 - y * warpf3));
          v +=
            warp * 0.0035 * Math.cos(warpTimeV * 0.375 - warpScaleInv * (x * warpf2 + y * warpf1));
          u +=
            warp * 0.0035 * Math.cos(warpTimeV * 0.753 - warpScaleInv * (x * warpf1 - y * warpf2));
          v +=
            warp * 0.0035 * Math.sin(warpTimeV * 0.825 + warpScaleInv * (x * warpf0 + y * warpf3));
        }
  ...
  render({ audioLevels, elapsedTime } = {}) {
    ...
    if (Math.abs(fpsRatio - 1.0) > 0.01) {
      ...
      mdVSFrame.echo_alpha = 1.0 - Math.pow(1.0 - mdVSFrame.echo_alpha, fpsRatio);
    }
