...
function _mwCheckShader(gl, shader, label) {
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.error(`[butterchurn] ${label} compile error:`, gl.getShaderInfoLog(shader));
    return false;
  }
  return true;
}
...
function _mwCheckProgram(gl, program, label) {
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.error(`[butterchurn] ${label} link error:`, gl.getProgramInfoLog(program));
    return false;
  }
  return true;
}
...
export default class CompShader {
  ...
  createShader(shaderText = '', _fallback = false) {
    let fragShaderText;
    ...
    /* [MW-PATCH: universal fps normalization] */
    fragShaderText = normalizeFpsConstants(fragShaderText);
    ...
    this.gl.shaderSource(
      ...
      `#version 300 es
                                      ...
                                      uniform float frame;
                                      uniform float fps;
                                      uniform float _mw_fps_ratio; /* [MW-PATCH: _mw_fps_ratio uniform] */
    ...
    const fragOk = _mwCheckShader(this.gl, fragShader, 'Comp fragment');
  ...
  static generateHueBase(mdVSFrame) {
    ...
    for (let i = 0; i < 4; i++) {
      hueBase[i * 3 + 0] =
        0.6 + 0.3 * Math.sin(mdVSFrame.time * 30.0 * 0.0143 + 3 + i * 21 + mdVSFrame.rand_start[3]);
      hueBase[i * 3 + 1] =
        0.6 + 0.3 * Math.sin(mdVSFrame.time * 30.0 * 0.0107 + 1 + i * 13 + mdVSFrame.rand_start[1]);
      hueBase[i * 3 + 2] =
        0.6 + 0.3 * Math.sin(mdVSFrame.time * 30.0 * 0.0129 + 6 + i * 9 + mdVSFrame.rand_start[2]);
  ...
  renderQuadTexture(
    ...
    this.gl.bufferData(this.gl.ARRAY_BUFFER, compColors, this.gl.DYNAMIC_DRAW);
    ...
    this.gl.uniform4fv(
      this.texsizeLoc,
      new Float32Array([this.texsizeX, this.texsizeY, 1.0 / this.texsizeX, 1.0 / this.texsizeY]),
    );
    ...
    this.gl.uniform1f(
      this.mwFpsRatioLoc,
      30.0 / mdVSFrame.fps,
    ); /* [MW-PATCH: _mw_fps_ratio uniform] */
